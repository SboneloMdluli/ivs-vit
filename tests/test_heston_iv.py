...
import implied_volatility_diffusion.synthetic.heston
...
def test_implied_vol_clamps_slightly_sub_intrinsic() -> None:
    ...
    iv = implied_volatility_diffusion.pricing.implied_vol.implied_volatility(intr - 1e-6, S, K, T, r)
    assert math.isfinite(iv) and iv > 0.0


def test_bs_implied_vol_roundtrip() -> None:
    S, K, T, r, sigma = 100.0, 100.0, 0.5, 0.03, 0.25
    price = float(implied_volatility_diffusion.pricing.black_scholes.bs_call_price(S, K, T, r, sigma))
    iv = implied_volatility_diffusion.pricing.implied_vol.implied_volatility(price, S, K, T, r)
...
def test_heston_degenerates_to_bs() -> None:
    ...
    price = implied_volatility_diffusion.models.heston.cos.heston_call_cos(S, K, T, r, 50.0, 0.04, 0.01, 0.0, 0.04)
    iv = implied_volatility_diffusion.pricing.implied_vol.implied_volatility(price, S, K, T, r)
...
def test_tau_extrapolate_below_aligns_short_maturity_column() -> None:
    ...
    _, tau, iv = implied_volatility_diffusion.synthetic.heston.implied_vol_surface_for_heston_params(params, cfg)
...
def test_load_config_and_lhs() -> None:
    root = Path(__file__).resolve().parents[1]
    cfg = implied_volatility_diffusion.synthetic.heston.load_heston_iv_surface_config(root / "config")
    params = implied_volatility_diffusion.synthetic.heston.lhs_heston_params(cfg, n_samples=5, seed=0)
...
def test_sequential_ivs_shape_and_state_override() -> None:
    root = Path(__file__).resolve().parents[1]
    cfg = implied_volatility_diffusion.synthetic.heston.load_heston_iv_surface_config(root / "config")
    cfg = implied_volatility_diffusion.config.merge_config(
        cfg,
        {
